...
bool CSkypeProto::LogIn()
{
	...
	if (this->skypeKit->GetAccount(::mir_u2a(this->login), this->account))
	{
...
void CSkypeProto::SetAccountSettings()
{
	...
	this->skypeKit->SetInt(SETUPKEY_PORT, port);
	...
	this->skypeKit->SetInt(SETUPKEY_DISABLE_PORT80, (int)!useAlternativePorts);
	...
	if (this->hNetLibUser)
	{
		...
		if (nlus.useProxy) 
		{
			...
			case PROXYTYPE_HTTPS:
				...
				this->skypeKit->SetInt(SETUPKEY_HTTPS_PROXY_ENABLE, 1);
				this->skypeKit->SetInt(SETUPKEY_SOCKS_PROXY_ENABLE, 0);
				this->skypeKit->SetStr(SETUPKEY_HTTPS_PROXY_ADDR, address);
				if (nlus.useProxyAuth)
				{
					char encodedPass[MAX_PATH];
					Base64::Encode(nlus.szProxyAuthPassword, encodedPass, MAX_PATH);

					this->skypeKit->SetStr(SETUPKEY_HTTPS_PROXY_USER,	nlus.szProxyAuthUser);
					this->skypeKit->SetStr(SETUPKEY_HTTPS_PROXY_PWD,	encodedPass);
				}
				break;
			...
			case PROXYTYPE_SOCKS5:
				...
				this->skypeKit->SetInt(SETUPKEY_HTTPS_PROXY_ENABLE, 0);
				this->skypeKit->SetInt(SETUPKEY_SOCKS_PROXY_ENABLE, 1);
				this->skypeKit->SetStr(SETUPKEY_SOCKS_PROXY_ADDR, address);
				if (nlus.useProxyAuth)
				{
					this->skypeKit->SetStr(SETUPKEY_SOCKS_PROXY_USER,	nlus.szProxyAuthUser);
					this->skypeKit->SetStr(SETUPKEY_SOCKS_PROXY_PWD,	nlus.szProxyAuthPassword);
				}
				break;

			default:
				this->Log(L"Setting automatic proxy detection");
				this->skypeKit->Delete(SETUPKEY_HTTPS_PROXY_ENABLE);
				this->skypeKit->Delete(SETUPKEY_HTTPS_PROXY_ADDR);
				this->skypeKit->Delete(SETUPKEY_HTTPS_PROXY_USER);
				this->skypeKit->Delete(SETUPKEY_HTTPS_PROXY_PWD);
				this->skypeKit->Delete(SETUPKEY_SOCKS_PROXY_ENABLE);
				this->skypeKit->Delete(SETUPKEY_SOCKS_PROXY_ADDR);
				this->skypeKit->Delete(SETUPKEY_SOCKS_PROXY_USER);
				this->skypeKit->Delete(SETUPKEY_SOCKS_PROXY_PWD);
				break;
			}
		}
	}
