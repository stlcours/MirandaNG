...
static char WaClassesMd5[] = "kI6dipxOAk055AWZxknGqg==";
...
std::string WAToken::GenerateToken(const string &number)
{
	...
	ptrA result(mir_base64_encode(hash2, sizeof(hash2)));
...
CMStringA WARegister::RequestCodeUrl(const std::string &phoneNumber)
{
	bool bResult = false;

	try {
		std::string id = GenerateIdentity(phoneNumber);
		std::string token = WAToken::GenerateToken(phoneNumber);

		PhoneNumber pn(phoneNumber);

		const char *cc = pn.CC.c_str(), *n = pn.Number.c_str();
		
		return CMStringA(FORMAT, "https://v.whatsapp.net/v2/code?cc=%s&in=%s&to=%s%s&method=sms&mcc=%03d&mnc=%03d&token=%s&id=%s&lg=%s&lc=%s",
							 cc, n, cc, n, pn.mcc, pn.mnc, token.c_str(), id.c_str(), pn.ISO639, pn.ISO3166);
	}
	catch (...)
	{}
