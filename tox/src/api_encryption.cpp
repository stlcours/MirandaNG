...
Tox *tox_encrypted_new(const struct Tox_Options *options, const uint8_t *data, size_t length, uint8_t *passphrase, size_t pplength, TOX_ERR_ENCRYPTED_NEW *error)
{
	return CreateFunction<Tox*(*)(const struct Tox_Options*, const uint8_t*, size_t, uint8_t*, size_t, TOX_ERR_ENCRYPTED_NEW*)>(__FUNCTION__)(options, data, length, passphrase, pplength, error);
}
