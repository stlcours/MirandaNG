...
public:
	...
	SteamResponseDelegate(CSteamProto *proto, SteamResponseWithArgCallback responseCallback, void *arg)
...
{
	((SteamResponseDelegate*)arg)->Invoke(response);
}
...
{
	SteamResponseDelegate *delegate = new SteamResponseDelegate(this, response, arg);
	requestQueue->Push(request, SteamHttpResponse, delegate, last);
}
...
{
	SteamResponseDelegate *delegate = new SteamResponseDelegate(this, response, arg);
	requestQueue->Send(request, SteamHttpResponse, delegate, last);
}
